...
const messageManager = {
      ...
      if (savedUser) {
        try {
          ...
          if (!currentUser || !currentUser.id || !currentUser.username) {
            ...
            // 向服务器验证用户状态
            const isValid = await this.verifyUserWithServer(currentUser.id);
            if (!isValid) {
              currentUser = null;
            }
          }
  ...
  updateStatistics: function() {
    ...
    if (followCountEl) followCountEl.textContent = followCount;
  },
  ...
  renderNotifications: function() {
    ...
    filteredNotifications.forEach(notification => {
      ...
      if (notification.type === 'like') {
        ...
        actionText = '关注了你';
        contentHtml = `
          <div class="notification-follow-content">
            点击查看 <a href="profile.html?id=${notification.fromUserId}" class="follow-link">TA的主页</a>
          </div>
        `;
        ...
        if (notification.systemType === 'post_deleted') {
          ...
          `;
        } else if (notification.systemType === 'report_result') {
          // 被举报结果通知
          actionText = '举报处理结果';
          contentHtml = `
            <div class="notification-system-content">
              <div class="notification-system-detail">
                ${this.escapeHtml(notification.message || `您的${notification.targetType || '内容'}因"${notification.reason || '违规'}"被举报，经核实违规属实`)}
              </div>
              ${notification.targetContent ? `
                <div class="notification-system-reason">
                  <strong>违规${notification.targetType || '内容'}：</strong>${this.escapeHtml(notification.targetContent)}
                </div>
              ` : ''}
              ${notification.banDays ? `
                <div class="notification-system-reason">
                  <strong>封禁时长：</strong>${notification.banDays === 365 ? '永久' : notification.banDays + '天'}
                </div>
              ` : ''}
            </div>
          `;
        } else if (notification.systemType === 'reporter_result') {
          // 举报人收到的处理结果通知
          actionText = '举报处理结果';
          const targetUser = notification.targetUsername || '用户';
          contentHtml = `
            <div class="notification-system-content">
              <div class="notification-system-detail">
                ${this.escapeHtml(notification.message || (notification.isApproved ? 
                  `您举报的用户"${targetUser}"因"${notification.reason || '违规'}"违规属实，已进行处理` : 
                  `您举报的内容经核实未发现违规`))}
              </div>
              ${notification.targetContent ? `
                <div class="notification-system-reason">
                  <strong>举报${notification.targetType || '内容'}：</strong>${this.escapeHtml(notification.targetContent)}
                </div>
              ` : ''}
              ${notification.isApproved && notification.banDays ? `
                <div class="notification-system-reason">
                  <strong>处理结果：</strong>封禁${notification.banDays === 365 ? '永久' : notification.banDays + '天'}
                </div>
              ` : ''}
              ${notification.note ? `
                <div class="notification-system-admin">
                  <strong>备注：</strong>${this.escapeHtml(notification.note)}
                </div>
              ` : ''}
            </div>
          `;
          ...
          contentHtml = `
            <div class="notification-system-content">
              <div class="notification-system-detail">
                ${this.escapeHtml(notification.message || '系统通知')}
              </div>
            </div>
          `;
        }
  ...
  verifyUserWithServer: async function(userId) {
    try {
      ...
      if (data.success && data.valid) {
        const localUser = JSON.parse(localStorage.getItem('forumUser') || '{}');
